...
export interface INonAcsiTeamState {
  search: string;
}
...
export class NonAcsiTeam extends React.Component<INonAcsiTeamProps, INonAcsiTeamState> {
  ...
  render() {
    const { nonAcsiTeamList, match } = this.props;
    return (
      <div>
        <h2 id="non-acsi-team-heading">
          Non Acsi Teams
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp; Create new Non Acsi Team
          </Link>
        </h2>
        <Row>
          <Col sm="12">
        ...
        <div className="table-responsive">
          {nonAcsiTeamList && nonAcsiTeamList.length > 0 ? (
            <Table responsive>
              ...
              <tbody>
                {nonAcsiTeamList.map((nonAcsiTeam, i) => (
                  <tr key={`entity-${i}`}>
                    <td>
                      <Button tag={Link} to={`${match.url}/${nonAcsiTeam.id}`} color="link" size="sm">
                        {nonAcsiTeam.id}
                      </Button>
                    </td>
                    ...
                    <td className="text-right">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button tag={Link} to={`${match.url}/${nonAcsiTeam.id}/edit`} color="primary" size="sm">
                          <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                        </Button>
                        <Button tag={Link} to={`${match.url}/${nonAcsiTeam.id}/delete`} color="danger" size="sm">
            ...
            <div className="alert alert-warning">No Non Acsi Teams found</div>
          )}
        </div>
      </div>
    );
