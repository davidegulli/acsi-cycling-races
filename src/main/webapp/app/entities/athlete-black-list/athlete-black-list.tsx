...
export interface IAthleteBlackListState {
  search: string;
}
...
export class AthleteBlackList extends React.Component<IAthleteBlackListProps, IAthleteBlackListState> {
  ...
  render() {
    const { athleteBlackListList, match } = this.props;
    return (
      <div>
        <h2 id="athlete-black-list-heading">
          Athlete Black Lists
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp; Create new Athlete Black List
          </Link>
        </h2>
        <Row>
          <Col sm="12">
        ...
        <div className="table-responsive">
          {athleteBlackListList && athleteBlackListList.length > 0 ? (
            <Table responsive>
              ...
              <tbody>
                {athleteBlackListList.map((athleteBlackList, i) => (
                  <tr key={`entity-${i}`}>
                    <td>
                      <Button tag={Link} to={`${match.url}/${athleteBlackList.id}`} color="link" size="sm">
                        {athleteBlackList.id}
                      </Button>
                    </td>
                    ...
                    <td>
                      <TextFormat type="date" value={athleteBlackList.birthDate} format={APP_DATE_FORMAT} />
                    </td>
                    <td>{athleteBlackList.taxCode}</td>
                    <td className="text-right">
                      <div className="btn-group flex-btn-group-container">
                        <Button tag={Link} to={`${match.url}/${athleteBlackList.id}`} color="info" size="sm">
                          <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                        </Button>
                        <Button tag={Link} to={`${match.url}/${athleteBlackList.id}/edit`} color="primary" size="sm">
                          <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                        </Button>
                        <Button tag={Link} to={`${match.url}/${athleteBlackList.id}/delete`} color="danger" size="sm">
            ...
            <div className="alert alert-warning">No Athlete Black Lists found</div>
          )}
        </div>
      </div>
    );
